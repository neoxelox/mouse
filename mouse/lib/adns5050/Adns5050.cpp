...
Adns5050::Adns5050(const int ncsPin, const int resetPin)
{
	...
	Serial.print("ADNS.NCS: ");
	Serial.println(_ncsPin);
	Serial.print("ADNS.NRESET: ");
	Serial.println(_resetPin);
#endif
}
...
bool Adns5050::begin(const unsigned long fsck)
{
	...
	if (read(Product_ID2) != ADNS_5050_Product_ID2)
		return false;
	if (read(Revision_ID) != ADNS_5050_Revision_ID)
		return false;
	if ((unsigned char)(~read(Inv_Rev_ID)) != ADNS_5050_Revision_ID)
		return false;
